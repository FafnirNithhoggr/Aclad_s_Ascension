...
public class CameraController : MonoBehaviour
{
    ...
    private void HandleMainToThird() {
        ...
        elapsedTime += Time.deltaTime;
        float factor = Mathf.SmoothStep(0, 1, elapsedTime / 2.0f) * Mathf.Max(1, ((elapsedTime - 1.0f)*5.0f) - 4.0f);
        ...
        if (Vector3.Distance(transform.position, targetPosition) < 0.06f && Quaternion.Angle(transform.rotation, targetRotation) < 1.0f) {
